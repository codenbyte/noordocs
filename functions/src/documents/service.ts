...
import { getSignatureProvider } from "./providers";
import type {
  NoorDocument,
  DocumentParticipant,
  AuditLogEntry,
...
import { sendEmail, reviewRequestEmail, reviewCompleteEmail } from "../email";
import type {
  CreateDocumentInput,
  UpdateDocumentInput,
  AddParticipantInput,
...
function toSummary(doc: NoorDocument): DocumentSummary {
  return {
    id: doc.id,
    type: doc.type,
    title: doc.title,
...
export async function createDocument(
  ...
  const timestamp = now();
  const participants: DocumentParticipant[] = input.participants.map((p) => ({
    id: participantId(),
    name: p.name,
    email: p.email,
    role: p.role,
  ...
  const docData: Omit<NoorDocument, "id"> = {
  ...
  const doc: NoorDocument = { id: ref.id, ...docData };
...
export async function updateDocument(
  ...
  return { ...doc, ...updates } as NoorDocument;
}

/**
 * Get a single document by ID. Throws if not found.
 */
export async function getDocumentById(documentId: string): Promise<NoorDocument> {
  const snap = await db.collection(DOCUMENTS_COL).doc(documentId).get();
  if (!snap.exists) {
    throw new ServiceError(404, "Document not found");
  }
  return { id: snap.id, ...snap.data() } as NoorDocument;
}

/**
 * Get document with full audit trail.
 */
export async function getDocumentWithAudit(
  documentId: string,
): Promise<{ document: NoorDocument; audit: AuditLogEntry[] }> {
 ...
 */
export async function listPendingReview(): Promise<DocumentSummary[]> {
  const snap = await db
    .collection(DOCUMENTS_COL)
  ...
  return snap.docs.map((d) => {
    const data = { id: d.id, ...d.data() } as NoorDocument;
...
export async function addParticipant(
  ...
  const participant: DocumentParticipant = {
    id: participantId(),
    name: input.name,
    email: input.email,
    role: input.role,
  ...
  if (reviewer) {
    ...
    try {
      ...
      if (reviewerDoc.email) {
        ...
        await sendEmail({ to: reviewerDoc.email, ...email });
      }
    } catch (emailErr) {
...
export async function reviewDocument(
  ...
  const AUDIT_MAP: Record<string, AuditAction> = {
  ...
  try {
    ...
    if (creatorUser.email) {
      const appUrl = process.env.APP_URL || "https://noorspace.app";
      const email = reviewCompleteEmail({
...
export async function sendForSignatures(
  ...
  const sigRequest: Omit<SignatureRequest, "id"> = {
    documentId,
...
export async function signDocument(
  ...
  // Check if all signed
  const allSigned = updatedParticipants.every(
    (p) => p.signatureStatus === "signed",
  );

  const newStatus: DocumentStatus = allSigned ? "completed" : "partially_signed";
...
export async function declineDocument(
  ...
  const updatedParticipants = doc.participants.map((p) =>
    p.id === input.participantId
      ? { ...p, signatureStatus: "declined" as const }
      : p,
  );
...
export async function handleSignatureWebhook(
  ...
  if (!event) return; // Internal provider or unrecognized event
  ...
  switch (event.type) {
    ...
    case "DECLINED": {
      ...
      if (!declinedParticipant || declinedParticipant.signatureStatus === "declined") break;
    ...
    case "EXPIRED":
      ...
      if (doc.status !== "archived" && doc.status !== "completed") {
        const expiredParticipants = doc.participants.map((p) =>
          p.signatureStatus === "pending"
            ? { ...p, signatureStatus: "expired" as const }
            : p,
        );
        await db.collection(DOCUMENTS_COL).doc(sigRequest.documentId).update({
        ...
        await appendAudit(
          ...
          "system",
          "Signature Provider",
          { reason: "Signing request expired" },
        );
      }
      break;
  }
